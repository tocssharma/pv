...
import ConfirmationDialog from '../ui/ConfirmationDialog1';
import HierarchicalEditor from './HierarchicalEditor';
import ProcessTreeView from './ProcessTreeView';
import EnhancedTreeViewer from './EnhancedTreeViewer7';
import ExcelProcessor from './ExcelProcessor';

import {DataViewer,HierarchicalView,RowDataView} from './importUtils';
const ExcelViewer = () => {
...
// Add unmounting check state
// Add unmounting check state
//const [isUnmounting, setIsUnmounting] = useState(false);
  
console.log("in ExcelViewer");
  useEffect(() => {
    if (initialData) {
      try {
...
useEffect(() => {
    return () => {
      ...
      setError(null);
          ...
          // If any pending operations, clean them up
          if (pendingData) {
...
<ConfirmationDialog
  ...
  onConfirm={handleConfirmDataUpdate}
  title="Confirm Data Processing"
>
  {dataChangesSummary && (
    <>
      <p>New data contains {dataChangesSummary.totalNewRows} rows
      {dataChangesSummary.difference !== 0 && (
        <span> ({dataChangesSummary.difference > 0 ? '+' : ''}{dataChangesSummary.difference} from current)</span>
      )}</p>
  ...
  <p className="mt-2">Do you want to proceed with processing this data?</p>
</ConfirmationDialog>
...
<ConfirmationDialog
  ...
  confirmText="Save"
>
  <p>Are you sure you want to save these changes to the database? This action cannot be undone.</p>
</ConfirmationDialog>
   ...
   {/* Show search and export only for table view */}
   {viewMode === 'table' && (
  ...
  <div className="flex gap-2">
    ...
    <button
      className={`px-4 py-2 rounded ${
        viewMode === 'enhanced-tree' 
          ? 'bg-blue-500 text-white' 
          : 'border border-gray-300 hover:bg-gray-50'
      }`}
      onClick={() => setViewMode('enhanced-tree')}
    >
      Process Hierarchy
    </button>
    <button
