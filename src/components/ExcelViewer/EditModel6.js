...
/* 
 const EditableRow = ({ 
    ...
    const handleDelete = () => {
      console.log("handleDelete:setShowDeleteDialog(true)");
      setShowDeleteDialog(true);
    };
  
    const handleConfirmDelete = async () => {
      console.log("handleDelete:handleConfirmDelete");
    ...
    return (
      ...
      `}>
        ...
        <ContextMenu onEdit={() => setShowEditForm(true)} onDelete={handleDelete}>
...
const EditModal = ({ isOpen, node, rows, onClose, onSave }) => {
    ...
    const [viewMode, setViewMode] = useState('table'); // 'table' or 'flow'
    ...
    <EditableRow
      ...
      onUpdate={(fieldOrUpdater) => {
        setEditableRows(prev => {
          const newRows = [...prev];
          const index = newRows.findIndex(r => r[levelConfig.id] === row[levelConfig.id]);
      ...
      validationErrors={validationErrors[rowId] || {}}
      allRows={editableRows}
      isDialogOpen={activeDialogs.has(rowId)}
      onDialogOpen={() => handleDialogOpen(rowId)}
      onDialogClose={() => handleDialogClose(rowId)}
    />
  );
})}
