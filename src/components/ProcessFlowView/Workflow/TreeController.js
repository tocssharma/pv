...
const useTreeStore = create((set, get) => ({
  ...
  initializeState: (expanded, node) => {
    console.log('Initializing state with:', { expanded, node });
    set({
...
const useTreeController = () => {
  ...
  const clickNode1 = (nodeId) => {
    ...
    const expandedNodes = [...parents, nodeId];
    console.log('Setting expanded nodes:', expandedNodes);
