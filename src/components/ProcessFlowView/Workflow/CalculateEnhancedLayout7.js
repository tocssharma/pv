...
export const calculateEnhancedLayout = (nodes, edges, direction = 'TB') => {
  ...
  console.log("Starting layout calculation with:", {
    totalNodes: nodes.length,
    totalEdges: edges.length,
    nodeIds: nodes.map(n => n.id),
    edgeConnections: edges.map(e => ({source: e.source, target: e.target}))
  });
  ...
  const initializeNodePositions = () => {
    console.log("Initializing positions for nodes");
    const nodeIds = new Set(nodes.map(n => n.id));
  
  edges.forEach(edge => {
    if (!nodeIds.has(edge.source)) {
      console.error('Edge source not in nodes:', {
    ...
    if (!nodeIds.has(edge.target)) {
      console.error('Edge target not in nodes:', {
        edgeSource: edge.source,
        edgeTarget: edge.target,
        availableNodes: Array.from(nodeIds)
      });
    ...
    console.log("Initializing positions for nodes:", nodes.map(n => n.id));
    ...
    nodes.forEach((node, index) => {
      ...
      if (typeof position.x !== 'number' || typeof position.y !== 'number') {
        console.error('Missing Invalid position calculated for node:', { 
          nodeId: node.id, 
          position,
    ...
    positions.forEach((pos, nodeId) => {
      if (!pos || typeof pos.x !== 'number' || typeof pos.y !== 'number') {
        console.error('Missing Invalid position found after initialization:', { nodeId, pos });
      }
    });
  ...
  for (let iteration = 0; iteration < ITERATIONS; iteration++) {
    ...
    nodes.forEach((node1, i) => {
      if (!nodePositions.has(node1.id)) {
        console.error('Missing position for node:', node1.id);
        return;
      }
      nodes.slice(i + 1).forEach(node2 => {
        if (!nodePositions.has(node2.id)) {
          console.error('Missing position for node:', node2.id);
          return;
        }
    ...
    edges.forEach(edge => {
      ...
      if (!nodes.find(n => n.id === edge.source)) {
        console.error('Edge references non-existent source node:', {
          edge,
          availableNodes: nodes.map(n => n.id)
        });
      }
      if (!nodes.find(n => n.id === edge.target)) {
        console.error('Edge references non-existent target node:', {
          edge,
          availableNodes: nodes.map(n => n.id)
        });
      }
      ...
      if (!nodePositions.has(edge.source) || !nodePositions.has(edge.target)) {
        console.error('Missing position for edge nodes:', {
          edge,
          sourceExists: nodePositions.has(edge.source),
          targetExists: nodePositions.has(edge.target)
        });
        return;
      }
...
export const calculateSimplifiedLayout = (nodes, edges, direction = 'TB') => {
  ...
  const routeEdges = (positions) => {
    ...
    return edges.map(edge => {
      ...
      const isValidationNode = sourceType === 'validation';
      const isDistributionNode = sourceType === 'distribution';
      const isNotFeasible = edge.data?.condition?.toLowerCase().includes('not');
...
export const calculateSimplifiedLayout_new = (nodes, edges, direction = 'TB') => {
  console.log("calculateSimplifiedLayout",{nodes, edges});
  // Constants for spacing
  const NODE_WIDTH = 480;
  const NODE_HEIGHT = 220;
  const EDGE_LENGTH = direction === 'TB' ? 250 : 250;
