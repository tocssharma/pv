...
console.log("CustomNode data:", data);
  ...
  const isValidation = type.toLowerCase === 'validation';
  const isDistribution = type.toLowerCase === 'distribution';
...
{/* Special Node Types Handling */
 ...
 <div className={getShapePositionClasses(layoutDirection).container}>
             <svg width="40" height="40" className={`transform ${layoutDirection === 'TB' ? 'rotate-45' : 'rotate-0'}`}>
              <rect x="8" y="8" width="24" height="24" 
                    className="fill-yellow-400 stroke-white" strokeWidth="2" />
            </svg>
            <EnhancedHandle
              type="source"
              position={layoutDirection === 'TB' ? Position.Left : Position.Top}
              id="validation-false"
              nodeId={id}
              isValidationNode={true}
              handleColor={colors.handle}
            ...
            <EnhancedHandle
              type="source"
              position={layoutDirection === 'TB' ? Position.Right : Position.Bottom}
              id="validation-true"
              nodeId={id}
              isValidationNode={true}
              handleColor={colors.handle}
