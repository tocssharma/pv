import React, { useRef, useState, useCallback, useLayoutEffect, useMemo,memo, useEffect  } from 'react';
import ReactFlow, {
  Background,
  Controls,
  MiniMap,
...
class ErrorBoundary extends React.Component {
  ...
  const useDynamicHandles = (nodeId, handles, setHandles) => {
    const addHandle = useCallback((position) => {
      const newHandle = {
      ...
      setHandles(prev => ({
        ...prev,
        [nodeId]: [...(prev[nodeId] || []), newHandle]
      }));
  
      return newHandle;
    }, [nodeId, setHandles]);

    const getOrCreateHandle = useCallback((position) => {
      const nodeHandles = handles[nodeId] || [];
      const existingHandle = nodeHandles.find(h => h.position === position);
      
      if (existingHandle) {
        return existingHandle;
      }
  
      return addHandle(position);
    }, [nodeId, handles, addHandle]);
...
const Flow = ({ 
  ...
  layoutDirection,fitView,project
}) => {
 ...
 const prevNodeCount=useRef(0);
  ...
  useEffect(() => {
    ...
    console.log("isInitialized.current && !isUserInteraction.current && nodes.length",isInitialized.current +":" + isUserInteraction.current +";"+ nodes.length);
...
useEffect(() => {
  ...
  // If the number of nodes changes significantly (e.g., loading a new diagram)
  const nodeCountChanged = Math.abs(nodes.length - prevNodeCount.current) > 1;
  if (nodeCountChanged) {
    resetAndFitView();
  }
  ...
  return (
    <div ref={reactFlowWrapper} className="h-full w-full">
      ...
      <LayoutToggle 
        ...
        7
      {/* Add a fit view button to the controls if needed */}
...
console.log("WorkflowDiagram:initialCollapsed",collapsedNodes);
  ...
  const handleLayoutDirectionChange = useCallback((newDirection) => {
    setLayoutDirection(newDirection);
    // Reset initialization to trigger re-layout
    setIsInitialized(false);
    
    // Allow time for layout to update before fitting view
    setTimeout(() => {
      setNodes((nodes) => {
        const { nodes: layoutedNodes } = calculateEnhancedLayout(nodes, edges, newDirection);
        return layoutedNodes;
      });
      
      // Add another small delay for the layout to be applied before fitting
      setTimeout(() => {
        fitView({
          padding: 0.2,
          duration: 800,
  ...
  console.log(`Creating edge ${edge.from}-${edge.to}:`, {
    sourceNode: processedNodes.find(n => n.id === edge.from),
    targetNode: processedNodes.find(n => n.id === edge.to),
    ...
    try {
      ...
      const { nodes: layoutedNodes, edges: routedEdges } = calculateEnhancedLayout(visibleNodes, visibleEdges, layoutDirection );
      // Batch state updates
